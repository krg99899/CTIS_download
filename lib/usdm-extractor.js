...
async function callGemini({ client, pdfBuffer, prompt, responseSchema, modelName = DEFAULT_MODEL, meter = null, stage = 'unknown', maxOutputTokens = 8192 }) {
  ...
  try {
    ...
    const msg = truncated
      ? `Gemini output hit the output token cap (truncated). Consider narrower page slices.`
  ...
  const studyDesign = {
    ...
    populations: [{
      ...
      plannedEnrollmentNumber: (() => {
        const n = parseInt(elig.plannedEnrollmentNumber ?? m.plannedEnrollment, 10);
        return Number.isFinite(n) && n > 0 ? n : null;
      })(),
      sex: codePair(elig.sexCode) || codePair('C49636'),
