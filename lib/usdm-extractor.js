...
// ──────────────────────────────────────────────────────────────────────
async function runSection({ client, pdfBuffer, toc, sectionKey, prompt, responseSchema, emit, fallbackFullIfEmpty = false }) {
  const section = toc.sections?.[sectionKey];
  const ranges = section?.ranges || [];

  let subPdf;
  if (ranges.length === 0) {
    if (!fallbackFullIfEmpty) return null;
    emit('progress', { stage: sectionKey, message: `No TOC range for ${sectionKey} — scanning full PDF.` });
    subPdf = pdfBuffer;
  } else {
    const label = ranges.map(r => `${r.startPage}-${r.endPage}${r.locationHint ? ` (${r.locationHint})` : ''}`).join(', ');
    emit('progress', { stage: sectionKey, message: `Extracting ${sectionKey} from pages ${label}` });
    subPdf = await splitter.sliceByRanges(pdfBuffer, ranges);
    if (!subPdf) return null;
  }

  try {
    return await callGemini({ client, pdfBuffer: subPdf, prompt, responseSchema });
  } catch (err) {
    emit('warning', { stage: sectionKey, message: `${sectionKey} extraction failed: ${err.message}` });
    return null;
  }
}
