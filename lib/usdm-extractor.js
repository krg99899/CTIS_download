...
async function callGemini({ client, pdfBuffer, prompt, responseSchema, modelName = DEFAULT_MODEL, meter = null, stage = 'unknown' }) {
  const contents = [
    {
      role: 'user',
      parts: [
...
// ──────────────────────────────────────────────────────────────────────
async function runSection({ client, pdfBuffer, toc, sectionKey, sectionLabel, prompt, responseSchema, emit, fallbackFullIfEmpty = false, meter = null }) {
  ...
  if (ranges.length === 0) {
    ...
    const result = await safeCallGemini({ client, pdfBuffer, prompt, responseSchema, emit, sectionKey, label, meter });
  ...
  // then merge per-section. If a single range is >8 pages, split it in half.
  const chunks = [];
  ...
  for (const r of chunks) {
    ...
    if (!subPdf) continue;
    const partial = await safeCallGemini({ client, pdfBuffer: subPdf, prompt, responseSchema, emit, sectionKey, label: `${label} ${r.startPage}–${r.endPage}`, meter });
...
async function safeCallGemini({ client, pdfBuffer, prompt, responseSchema, emit, sectionKey, label, meter }) {
  try {
    return await callGemini({ client, pdfBuffer, prompt, responseSchema, meter, stage: sectionKey });
  } catch (err) {
