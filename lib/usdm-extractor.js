...
async function runSection({ client, pdfBuffer, toc, sectionKey, sectionLabel, prompt, responseSchema, emit, fallbackFullIfEmpty = false }) {
  ...
  if (ranges.length === 0) {
    ...
    const result = await safeCallGemini({ client, pdfBuffer, prompt, responseSchema, emit, sectionKey, label });
  ...
  for (const r of chunks) {
    ...
    if (!subPdf) continue;
    const partial = await safeCallGemini({ client, pdfBuffer: subPdf, prompt, responseSchema, emit, sectionKey, label: `${label} ${r.startPage}–${r.endPage}` });
...
async function safeCallGemini({ client, pdfBuffer, prompt, responseSchema, emit, sectionKey, label }) {
  try {
    return await callGemini({ client, pdfBuffer, prompt, responseSchema });
  } catch (err) {
